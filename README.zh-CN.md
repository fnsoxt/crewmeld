...
K8s 沙箱部署：工具实例代码运行在独立的 Kubernetes Pod 中，与平台主进程隔离；支持 JavaScript 与 Python；故障 Pod 不影响平台稳定性。

预热池（Warm Pool）：平台预创建若干空闲 Pod，新工具实例上线时从池中取用并动态注入代码，缩短部署时延。依赖通过共享缓存复用。
...
└─────────────────────────────────────────────────────────────┘
                          ↓ LLM 决策调用工具
┌─────────────────────────────────────────────────────────────┐
│  工具层 │ K8s 沙箱独立 Node.js / Python 进程，代码隔离       │
...
工具沙箱依赖（仅当启用 `--profile k3s` 工具沙箱时需要）：

| 变量 | 说明 |
|------|------|
| `K8S_API_SERVER` | Kubernetes API Server 地址 |
| `K8S_API_TOKEN` | Kubernetes Bearer Token |
| `K8S_NODE_IP` | NodePort 访问的节点 IP |
| `K8S_DEPLOY_NAMESPACE` | 工具 Pod 所在命名空间（默认 `crewmeld-skills`） |
...
./start.sh --profile minio --profile k3s                     # 加工具沙箱
./start.sh --profile ragflow                                 # 加知识库
./start.sh --profile k3s --profile minio --profile ragflow   # 全部组件
```
...
| 加工具沙箱 | `--profile k3s --profile minio` | ~1.5 GB |
| 加知识库 | `--profile ragflow` | ~6 GB |
| 全部组件 | `--profile k3s --profile minio --profile ragflow` | ~7 GB |
