...
describe('Model Capabilities', () => {
  describe('supportsTemperature', () => {
    ...
    it.concurrent('should return false for models not in the temperature support registry', () => {
      expect(supportsTemperature('totally-unknown-model-xyz')).toBe(false)
      expect(supportsTemperature('nonexistent-llm')).toBe(false)
    })
  })
  ...
  describe('getMaxTemperature', () => {
    ...
    it.concurrent('should return 1 for models with temperature range 0-1', () => {
      const modelsRange01 = ['claude-sonnet-4-0', 'claude-opus-4-0', 'claude-3-7-sonnet-latest']
    ...
    it.concurrent('should return undefined for unknown models not in the registry', () => {
      expect(getMaxTemperature('totally-unknown-model-xyz')).toBeUndefined()
      expect(getMaxTemperature('nonexistent-llm')).toBeUndefined()
    })
  })

  describe('supportsToolUsageControl', () => {
    it.concurrent('should return true for providers that support tool usage control', () => {
      const supportedProviders = ['openai', 'anthropic', 'deepseek', 'google']
  ...
  describe('Reasoning Effort Values Per Model', () => {
    ...
    it.concurrent('should return null for models not in the reasoning effort registry', () => {
      expect(getReasoningEffortValuesForModel('qwen-max')).toBeNull()
      expect(getReasoningEffortValuesForModel('ernie-4.0')).toBeNull()
    })
  })
...
describe('Model Pricing Validation', () => {
  ...
  it.concurrent('should have correct pricing for o3', () => {
    const o3 = getModelPricing('o3')
    expect(o3).toBeDefined()
    expect(o3.input).toBe(2.0)
    expect(o3.output).toBe(8.0)
...
describe('Provider Management', () => {
  describe('getProviderFromModel', () => {
    it.concurrent('should return correct provider for known models', () => {
      ...
      expect(getProviderFromModel('deepseek-v3')).toBe('deepseek')
    })
