...
/**
 * Extract all URLs from [Attachment: ... url=<url> ...] annotations in userMessage
 * These URLs are generated by conversation/engine.ts via createToolAccessibleUrl during file upload
 */
function extractAttachmentUrls(text: string): string[] {
  if (!text) return []
  const urls: string[] = []
  const annotationRe = /\[Attachment:[^\]]+\]/g
