...
export const buildTimeCSPDirectives: CSPDirectives = {
  ...
  'script-src': [
    "'self'",
    // unsafe-inline kept: Next.js App Router runtime requires inline scripts (removal needs nonce mechanism)
    "'unsafe-inline'",
    // unsafe-eval only kept in dev mode (needed for HMR), removed in production to prevent XSS
    ...(isDev ? ["'unsafe-eval'"] : []),
    'https://*.google.com',
  ...
  'connect-src': [
    ...
    ...(env.NEXT_PUBLIC_SOCKET_URL
      ...
      : isDev
        ? ['http://localhost:6102', 'ws://localhost:6102']
        : []),
...
export function generateRuntimeCSP(): string {
  ...
  const socketUrl = getEnv('NEXT_PUBLIC_SOCKET_URL') || (isDev ? 'http://localhost:6102' : '')
  ...
  return `
    default-src 'self';
    script-src 'self' 'unsafe-inline' ${isDev ? "'unsafe-eval'" : ''} https://*.google.com https://apis.google.com https://assets.onedollarstats.com ${reactGrabScript};
    style-src 'self' 'unsafe-inline' https://fonts.googleapis.com;
...
export function getWorkflowExecutionCSPPolicy(): string {
  // Workflow execution needs unsafe-eval (dynamic code) and permissive connect-src (calling external APIs)
  // but restricts default-src to avoid being fully open
  return "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; connect-src *; img-src 'self' data: blob: https:; object-src 'none';"
}
