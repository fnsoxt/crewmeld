...
import { AlertTriangle, User, UserCircle, X } from 'lucide-react'
...
export const SopHumanEmployeeNode = React.memo(function SopHumanEmployeeNode({
  data,
  id,
  selected,
  ...
  return (
    <div
      ...
      className={`group relative w-[220px] rounded-lg border bg-white shadow-sm transition-shadow ${
        selected
          ? 'border-amber-500 shadow-md ring-1 ring-amber-200'
          : 'border-gray-200 hover:shadow-md'
      }`}
    >
      {/* BPMN style: top-left user task icon */}
      <div className='-left-1 -top-1 absolute rounded-tl-md rounded-br-md bg-amber-500 p-0.5'>
        ...
        <div className='mb-1.5 flex items-center justify-between'>
          <div className='flex min-w-0 items-center gap-1.5'>
            ...
            <span className='truncate font-medium text-gray-900 text-sm'>
              {sopNode.name || t('sops.nodeUnnamedHumanEmployee')}
            </span>
          </div>
          {onDelete && (
            <button
