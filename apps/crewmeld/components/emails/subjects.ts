...
/** Union of all email template identifiers supported by the platform. */
...
/**
 * Returns the email subject line for a given template type.
 * Subject strings are composed at call-time so they always reflect
 * the current brand configuration.
 *
 * @param type - Identifier for the email template being sent
 * @returns Localised subject line string
 */
export function getEmailSubject(type: EmailSubjectType): string {
  const { name: brandName } = getBrandConfig()
