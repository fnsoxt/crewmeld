...
import { registerBackgroundWorkers, shutdownBackgroundWorkers } from '@/socket/workers'
...
async function main() {
  ...
  // Prefer SOCKET_PORT over PORT — PORT is typically reserved for the main
  // Next.js app (6100), while SOCKET_PORT (6102) is socket-exclusive.
  const PORT = Number(env.SOCKET_PORT || env.PORT || 6102)
  ...
  // Start BullMQ workers that consume background jobs (knowledge,
  // schedules, webhooks, workflow runs, workspace notifications).
  ...
  const shutdown = async () => {
    ...
    try {
      ...
      logger.error('Error during background workers shutdown:', error)
    }
