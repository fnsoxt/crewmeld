...
This Helm chart deploys CrewMeld, a lightweight AI agent workflow platform, on Kubernetes.
...
```bash
helm install crewmeld ./helm/crewmeld -f custom-values.yaml
```
...
```bash
helm install crewmeld-dev ./helm/crewmeld \
  --values ./helm/crewmeld/examples/values-development.yaml \
  --namespace crewmeld-dev --create-namespace
```

### Production Environment

```bash
helm install crewmeld-prod ./helm/crewmeld \
  --values ./helm/crewmeld/examples/values-production.yaml \
  --namespace crewmeld-prod --create-namespace
```

### Azure Environment

```bash
helm install crewmeld-azure ./helm/crewmeld \
  --values ./helm/crewmeld/examples/values-azure.yaml \
...
```bash
helm install crewmeld-aws ./helm/crewmeld \
  --values ./helm/crewmeld/examples/values-aws.yaml \
  --namespace crewmeld --create-namespace
```
...
```bash
helm install crewmeld-gcp ./helm/crewmeld \
  --values ./helm/crewmeld/examples/values-gcp.yaml \
  --namespace crewmeld --create-namespace
```
...
```bash
helm install crewmeld-prod ./helm/crewmeld \
  --values ./helm/crewmeld/examples/values-external-db.yaml \
  --set externalDatabase.host="your-rds-endpoint.com" \
  --set externalDatabase.username="crewmeld_user" \
  --set externalDatabase.password="secure-password" \
  --set externalDatabase.database="crewmeld_prod" \
  --namespace crewmeld --create-namespace
```
...
| `global.useRegistryForAllImages` | Use custom registry for all images (not just proinsight/crewmeld*) | `false` |
...
| `app.image.repository` | App image repository | `proinsight/crewmeld` |
...
| `realtime.image.repository` | Realtime image repository | `proinsight/crewmeld-realtime` |
...
| `postgresql.auth.database` | PostgreSQL database name | `crewmeld` |
...
| `externalDatabase.database` | External database name | `crewmeld` |
...
| `ingress.app.host` | App ingress hostname | `crewmeld.local` |
| `ingress.app.paths` | App ingress paths | `[{path: "/", pathType: "Prefix"}]` |
| `ingress.realtime.host` | Realtime ingress hostname | `crewmeld-ws.local` |
| `ingress.realtime.paths` | Realtime ingress paths | `[{path: "/", pathType: "Prefix"}]` |
| `ingress.tls.enabled` | Enable TLS for ingress | `false` |
| `ingress.tls.secretName` | TLS secret name | `crewmeld-tls-secret` |
...
| `migrations.image.repository` | Migrations image repository | `proinsight/crewmeld-migrations` |
...
- Automatic service discovery for CrewMeld components
...
- Allow communication between CrewMeld components
...
```bash
helm upgrade crewmeld ./helm/crewmeld
```
...
externalSecrets:
  ...
  remoteRefs:
    app:
      BETTER_AUTH_SECRET: "crewmeld/app/better-auth-secret"
      ENCRYPTION_KEY: "crewmeld/app/encryption-key"
      INTERNAL_API_SECRET: "crewmeld/app/internal-api-secret"
    postgresql:
      password: "crewmeld/postgresql/password"
```
...
ingress:
  ...
  tls:
    enabled: true
    secretName: "crewmeld-tls-secret"
```
...
- Documentation: https://github.com/proinsight-io/crewmeld/blob/main/README.md
- GitHub Issues: https://github.com/proinsight-io/crewmeld/issues
